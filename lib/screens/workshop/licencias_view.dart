...
class LicenciasDocenteScreen extends StatefulWidget {
  const LicenciasDocenteScreen({super.key});

  @override
  _LicenciasDocenteScreenState createState() => _LicenciasDocenteScreenState();
}

class _LicenciasDocenteScreenState extends State<LicenciasDocenteScreen> {
  late List<Licencia> licencias = [];
  late Future<void> futureLicencias;
  ...
  void initState() {
    super.initState();
    futureLicencias = fetchLicencias();
    fetchMaterias();
    fetchProgramaciones();
    _loadUserData();
  }
  ...
  Future<void> fetchLicencias() async {
    final prefs = await SharedPreferences.getInstance();
    ...
    final response =
        await http.get(Uri.parse('http://165.227.100.249/licencias/'));

    if (response.statusCode == 200) {
      List jsonResponse = json.decode(response.body);
      setState(() {
        licencias = jsonResponse
            .map((licencia) => Licencia.fromJson(licencia))
            .where((licencia) => licencia.docenteId == docenteId)
            .toList();
      });
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Licencias del Docente'),
        backgroundColor: const Color.fromARGB(255, 4, 91, 108),
      ),
      drawer: _buildDrawer(),
      body: FutureBuilder<void>(
        future: futureLicencias,
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                var licencia = licencias[index];
                var materiaNombre =
                    getMateriaNombre(licencia.programacionAcademicaId);
                var grupoNombre =
                    getGrupoNombre(licencia.programacionAcademicaId);
                var fechaFormateada = formatFecha(licencia.fecha);
...
class Licencia {
  ...
  factory Licencia.fromJson(Map<String, dynamic> json) {
    return Licencia(
      ...
      fecha: json['fecha'],
      motivo: json['motivo'],
      fotoLicencia: json['fotoLicencia'],
    );
  }
}
